...
<!--\\\\\\\\\\\\\\\\\\\\\///////////////////////////////\\\\\\\\\\\\\\\\/////////\\\\\\ -->
  ...
  <div class="container-spacy   ">
   <div class="row ">
	  <h3 class="h4_user_inactive">View All Users</h3>
	<ul>
	...
	<br>
<?php
	$userlist="SELECT userid,username FROM customer ORDER BY userid LIMIT ?,?";
	$getlist=$db->prepare($userlist);
	$getlist->bind_param("ii",$rowstart,$rowcount);
	$getlist->execute();
	$getlist->store_result();
	$getlist->bind_result($usertoedit, $username);
	while ($getlist->fetch()) {
?>

 <p><?php echo $usertoedit." - ".$username;?></p>
	<?php
	echo '<a href="adminuseredit.php?usertoedit='.$usertoedit.'"class="button">Edit User</a>'; 
	echo '<a href="adminuserdelete.php?usertoedit='.$usertoedit.'"class="button">Delete User</a>'; 

	?>
<?php
	}
	$getlist->close();
	$totalrowssql="select count(*) as totalrows from customer;";
		...
		echo "<footer><p>Showing results $displayrowstart to $displayrowend of $totalrows</p><p>";
			if ($rowstart>0) {
				$newrowstart=(($rowstart-$rowcount)<0)?0:$rowstart-$rowcount;
					echo "<a href='admin_user_select2.php?f=$newrowstart&n=$rowcount'>&lt;&lt;Previous</a>";
				}
			...
			if (($rowcount+$rowstart)<$totalrows) {
				$newrowstart=(($rowstart+$rowcount)>=$totalrows)?$rowstart:$rowstart+$rowcount;
					echo "<a href='admin_user_select2.php?f=$newrowstart&n=$rowcount'>Next&gt;&gt;</a>";
				}
		echo "</p></footer>";
